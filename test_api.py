...
def test_list_tools(client):
    ...
    assert "cooling-priority-zones" in tool_ids
    assert "area-perimeter" in tool_ids
    assert "nearest-feature" in tool_ids
...
def test_run_cooling_priority_zones(client):
    r = client.post(
        "/api/v1/tools/cooling-priority-zones/run",
        json={
            "geojson": {
    ...
    assert data["execution"]["tool_id"] == "cooling-priority-zones"
    feature = data["result"]["features"][0]
    props = feature["properties"]

    assert feature["geometry"]["type"] == "Polygon"
    assert isinstance(props["cooling_priority_score"], float)
    assert props["cooling_priority_level"] in {"low", "medium", "high"}
    assert props["top_driver"] in {
        "tree_canopy",
        "impervious",
        "population_density",
        "vulnerability",
...
def test_run_area_perimeter(client):
    r = client.post(
        "/api/v1/tools/area-perimeter/run",
        json={
            "geojson": {
                "type": "FeatureCollection",
                "features": [
    ...
    assert r.status_code == 200
    data = r.json()
    assert data["execution"]["tool_id"] == "area-perimeter"
    assert data["execution"]["status"] == "success"
